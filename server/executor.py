...
def execute_sql(sql: str, row_limit: int = 1000, timeout: float = 5.0) -> Tuple[List[str], List[Tuple[Any]]]:
    sql_clean = sql.lstrip().lower()

    # SELECT: read-only connection + limit
    ...
    # INSERT (write): explicit support only
    if not sql_clean.startswith("insert"):
        raise ValueError("Only SELECT and INSERT statements are supported")
